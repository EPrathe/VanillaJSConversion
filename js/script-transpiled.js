...
(function () {
  ...
  const apiKey = "def9dbc1ee612af17f7ba79a9377a211"; // Replace "APIKEY" with your own API key; otherwise, your HTTP request will not work
  ...
  let state = {};
  let category = 'all'; // get weather data when user clicks Forecast button, then add temp & conditions to view
  //	$('.forecast-button').click(function(e) {

  document.querySelector('.forecast-button').addEventListener('click', function (e) {
    e.preventDefault(); //		const location = $('#location').val();

    const location = document.querySelector('#location').value; //		$('#location').val('');
  ...
  }, false); // update list of sports when user selects a different category (solo/team/all)
  ...
  function updateUISuccess(response) {
    ...
    const into = document.querySelector('.conditions'); //		ReactDOM.render(<Forecast {...state} />, $into);
    ...
    */
    ...
    let conditionsPara = document.createElement('p'); //conditionsPara.textContent = state.degCInt + '\u00B0 C / ' + state.degFInt + '\u00B0 F';
  ...
  function updateActivityList(event) {
    ...
    if (event !== undefined && event.target.classList.contains('selected')) {
      ...
      //   or change the DOM
      return true; //		} else if (event !== undefined && !$(this).hasClass('selected')) {
      ...
      category = event.target.id; //			$('.options div').removeClass('selected');
    ...
    const into = document.querySelector('.activities'); //		ReactDOM.render(<Activities {...state} />, $into);
